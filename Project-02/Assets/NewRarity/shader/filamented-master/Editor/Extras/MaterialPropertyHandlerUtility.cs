...
// https://github.com/momoma-null/GeneLit/blob/b756857d7c83b63e760b5facb44d616169edac3b/Editor/SingleLineDrawer.cs
namespace Silent.FilamentedExtras.Unity
{
    static class MaterialPropertyHandlerUtility
    {
        ...
        public static void ReplacePropertyDrawerWithDecorator(this MaterialProperty prop, MaterialPropertyDrawer newDecorator)
        {
            var handler = GetHandler(prop);
            s_m_PropertyDrawerInfo.SetValue(handler, newDecorator);
            var decorators = s_m_DecoratorDrawersInfo.GetValue(handler) as List<MaterialPropertyDrawer>;
            decorators.Clear();
            s_m_DecoratorDrawersInfo.SetValue(handler, decorators);
        }
